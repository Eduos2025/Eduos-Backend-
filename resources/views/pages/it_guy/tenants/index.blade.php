                                ...
                                <div class="list-icons">
                                    <div class="dropdown">
                                        ...
                                        <div class="dropdown-menu dropdown-menu-left">
                                            ...
                                            {{-- Delete --}}
                                            @if(Qs::userIsHead())
                                            <a id="tenant-delete-{{ Qs::hash($tenant->id) }}" onclick="confirmPermanentDeleteTwice(this.id)" href="javascript:;" class="dropdown-item text-danger"><i class="material-symbols-rounded">delete</i> Delete</a>
                                            <form method="post" id="item-delete-tenant-delete-{{ Qs::hash($tenant->id) }}" action="{{ route('tenants.destroy', Qs::hash($tenant->id)) }}" class="hidden page-block">@csrf @method('delete')</form>
                                            {{-- Tenants Migrate Fresh --}}
                                            <a id="tenant-migrate-fresh-{{ Qs::hash($tenant->id) }}" onclick="confirmOperation(this.id)" href="javascript:;" class="dropdown-item text-warning"><i class="material-symbols-rounded">swap_horiz</i> Migrate Fresh</a>
                                            <form method="post" id="item-confirm-operation-tenant-migrate-fresh-{{ Qs::hash($tenant->id) }}" onclick="confirmOperation(this.id)" action="{{ route('tenants.run_command', [1, Qs::hash($tenant->id)]) }}" class="hidden page-block">@csrf @method('post')</form>
                                            {{-- Tenants Seed --}}
                                            <a id="tenant-seed-{{ Qs::hash($tenant->id) }}" onclick="confirmOperation(this.id)" href="javascript:;" class="dropdown-item text-warning"><i class="material-symbols-rounded">filter_vintage</i> Seed</a>
                                            <form method="post" id="item-confirm-operation-tenant-seed-{{ Qs::hash($tenant->id) }}" onclick="confirmOperation(this.id)" action="{{ route('tenants.run_command', [2, Qs::hash($tenant->id)]) }}" class="hidden page-block">@csrf @method('post')</form>
                                            @endif
                                        </div>
                                    </div>
                                </div>

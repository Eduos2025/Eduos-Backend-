...
const blockUI = (message = "Please wait...", textColor = "orange") =>
    $.blockUI({
        message: `<i class="mr-1 m-auto material-symbols-rounded spinner">progress_activity</i> <span class="align-middle">${message}</span>`,
        css: {
            padding: "3px",
            color: textColor,
...
function pop(data) {
    Toast.fire({
        ...
        text: data.msg,
        icon: data.type,
        timer: data.timer ?? pop_timer, // You can set timer to 0 to prevent autohide (pop_timer defined in custom.blade.php)
        showConfirmButton: false,
    });
}
